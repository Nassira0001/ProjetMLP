...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'backpropagation'))
...
model = MLP([2, 64, 64, 1], activation='linear', output_activation='linear', init_method='he')

print("Entraînement avec activation linéaire...")
losses = train(model, X_norm, y_norm, epochs=500, learning_rate=0.01, batch_size=64)

plot_loss(losses, title="Loss avec activation linéaire", save_path="loss_linear.png")
...
print(f"MSE avec activation linéaire : {mse:.6f}")
...
plot_side_by_side(Z_true, Z_pred, x_vals, y_vals,
                  ...
                  title_pred="Prédictions (activations linéaires)",
                  save_path="comparison_linear.png")